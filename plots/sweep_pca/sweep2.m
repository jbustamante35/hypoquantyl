...
% This function outputs a single plot of the original synthetic contour 
% (dotted black line) and a single step up (solid green line) or down (solid
% red line) defined by inputted function handles.
%
% This function performs an iterative step up and down through a single
% principal component, where the iterative step is defined by user input.
% User also determines which principal component to iterate through.
%
...
%
% See the performSweep function to use this to sweep through all principal
% components for multiple iterative steps. [or read help performSweep]
%
% Usage:
%   [scoresStruct, simsStruct] = ...
%       sweep2(pcaX, pcaY, dim2chg, pc2chg, upFn, dwnFn, stp, f)
%
% Input:
...
%   dim2chg: [1|2] to iteratively change [x-coordinate|y-coordinate] PCs
...
%
% Output:
%   scoresStruct: structure containing PC values after iterative step
%       up: PC values after upFn
%       mean: mean PC values from dataset
%       down: PC values after dwnFn
%   simsStruct: structure containing synthetic values after transformations
...
% Check if using full structures or individual vectors
...
%% Run pcaSweep for PCA data for x-/y-coordinates [hold values of opposite]
