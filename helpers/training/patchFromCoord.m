function [P, buffVert] = patchFromCoord(seg, pm, mid, img, buff, sz, ver)
...
switch ver
    case 'scaled'
        %% Scale cropbox to area the segment takes
        ...
        pxls = cellfun(@(x) mapCurve2Image(x, img, pm, mid), ...
            crds, 'UniformOutput', 0);
        P    = cat(2, pxls{:});
        
    case 'fixed'
        %% Set fixed size of cropbox around midpoint and rotate around midpoint
        
        
    otherwise
        fprintf(2, 'Select algorithm for getting cropbox [scaled|fixed]\n');
        P        = [];
        buffVert = [];
        return;
end
