...
%% reverseEnvelopeMethod: revert envelope coordinates to normalized Curve segment coordinates
% This function is the reverse of envelopeMethod, which takes coordinates from a Curve object's
% NormalSegments and represents it as coordinates within it's envelope. Envelope coordinates are
% represented as [x y] where x is the normalized distance [0, 1] between start and end points, and y
% is the normalized distance [-1, 1] from the center segment, between the two envelopes.
%
% Converting the an envelope's y-coordinate to it's corresponding midpoint-normalized y-coordinate
% is the same equation for computing the distance from the curve (see help distanceFromCurve), but
% solving for the env_norm_y variable instead of the env_env_y variable:
...
%   seg: original segment in NormalSegments representation
%   sz: distance from original segment to each envelope
%
% Output:
%   crd: segment coordinates in NormalSegments representation
%
